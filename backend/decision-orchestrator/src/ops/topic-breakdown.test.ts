...
import { aggregateByTopic, getTopRefusalReasons, TopicCounter } from './topic-breakdown';

// Test helper to create mock event records
function createIssuedEvent(topicId: string): EventRecord {
  return {
...
function createRefusedEvent(topicId: string, reason: string): EventRecord {
  return {
    event_id: `evt_test_${Math.random().toString(36).slice(2)}`,
    created_at: new Date().toISOString(),
...
test('aggregateByTopic returns counters for all known topics', () => {
  ...
  const { counters } = aggregateByTopic([], [], knownTopics);
  assertEqual(counters.length, 3, 'counter count');
  assertEqual(
    counters.map((c) => c.topic_id).sort(),
    knownTopics.sort(),
    'topic IDs'
  );
});
...
test('aggregateByTopic counts issued events correctly', () => {
  ...
  const { counters } = aggregateByTopic(issued, [], knownTopics);
...
test('aggregateByTopic counts refused events correctly', () => {
  ...
  const { counters, refusalReasons } = aggregateByTopic([], refused, knownTopics);
...
test('aggregateByTopic calculates refusal rate correctly', () => {
  ...
  const { counters } = aggregateByTopic(issued, refused, knownTopics);
...
test('getTopRefusalReasons returns sorted top N', () => {
  ...
  const top3 = getTopRefusalReasons(reasons, 3);
...
test('getTopRefusalReasons handles empty map', () => {
  const reasons = new Map<string, number>();
  const topN = getTopRefusalReasons(reasons, 5);
...
test('aggregateByTopic extracts topic from session_id pattern', () => {
  ...
  const { counters } = aggregateByTopic([event], [], knownTopics);
