...
exports.isDefaultInput = isDefaultInput;
exports.getSnapshot = getSnapshot;
exports.acquireLock = acquireLock;
exports.storeSnapshot = storeSnapshot;
exports.releaseLock = releaseLock;
exports.invalidateSnapshot = invalidateSnapshot;
...
function hashInputs(input) {
    ...
    return (0, crypto_1.createHash)('sha256').update(JSON.stringify(relevant)).digest('hex').substring(0, 16);
}
/**
...
function extractTopicId(input) {
    ...
    return (0, crypto_1.createHash)('sha256').update(key).digest('hex').substring(0, 12);
}
/**
 * Check if an input is using default values (cacheable)
 * Custom/override inputs should bypass the cache
 */
function isDefaultInput(input) {
    // Default inputs have unknown/balanced values
    const ctx = input.user_context;
    return (ctx.traveler_type === 'unknown' ||
        ctx.traveler_type === 'first_time') && (ctx.budget_band === 'unknown' ||
        ctx.budget_band === 'fair_value') && (ctx.dates.type === 'unknown' ||
 ...
 */
async function getSnapshot(topicId, inputsHash) {
    try {
 ...
 */
async function acquireLock(topicId) {
    ...
    try {
        await docClient.send(new lib_dynamodb_1.UpdateCommand({
            ...
            ConditionExpression: 'attribute_not_exists(lock_until) OR lock_until < :now OR lock_id = :lockId',
 ...
 */
async function storeSnapshot(topicId, response, inputsHash, lockId) {
    const now = new Date();
 ...
 */
async function releaseLock(topicId, lockId) {
    try {
        await docClient.send(new lib_dynamodb_1.UpdateCommand({
            TableName: SNAPSHOT_TABLE,
            Key: { topic_id: topicId },
 ...
 */
async function invalidateSnapshot(topicId) {
    try {
