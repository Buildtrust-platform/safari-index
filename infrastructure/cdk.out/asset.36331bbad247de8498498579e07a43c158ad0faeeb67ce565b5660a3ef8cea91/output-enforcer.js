...
exports.detectForbiddenPhrases = detectForbiddenPhrases;
exports.detectGuarantees = detectGuarantees;
exports.detectSelfReference = detectSelfReference;
exports.detectEmojis = detectEmojis;
exports.detectExclamationMarks = detectExclamationMarks;
exports.validateAIOutput = validateAIOutput;
exports.generateRetryPrompt = generateRetryPrompt;
 ...
 */
function detectGuarantees(text) {
    const violations = [];
 ...
 */
function detectSelfReference(text) {
    const violations = [];
 ...
 */
function detectExclamationMarks(text) {
 ...
 */
function validateDecisionOutput(output, errors) {
    ...
    if (!decision.assumptions || !Array.isArray(decision.assumptions)) {
        ...
        for (let i = 0; i < decision.assumptions.length; i++) {
            ...
            if (typeof assumption.confidence !== 'number' ||
    ...
    else if (decision.change_conditions.length < 2 ||
 ...
 */
function validateRefusalOutput(output, errors) {
    ...
    if (!refusal.missing_or_conflicting_inputs ||
    ...
    else if (refusal.missing_or_conflicting_inputs.length < 2 ||
 ...
 */
function validateClarificationOutput(output, errors) {
    ...
    else if (clarification.questions.length < 1 ||
 ...
 */
function validateTradeoffExplanationOutput(output, errors) {
 ...
 */
function validateRevisionOutput(output, errors) {
    ...
    if (!revision.decision) {
        ...
        validateDecisionOutput({ type: 'decision', decision: revision.decision }, errors.map((e) => ({
            ...e,
    ...
    if (!output.type) {
        return {
            ...
            errors: [
                {
                    field: 'type',
                    message: 'Output must have a type: decision, refusal, clarification, tradeoff_explanation, or revision',
                },
            ],
        };
    }
    ...
    switch (output.type) {
        ...
        case 'refusal':
            if (!output.refusal) {
                ...
                validateRefusalOutput(output, errors);
            }
            break;
        case 'clarification':
            if (!output.clarification) {
                ...
                validateClarificationOutput(output, errors);
            }
            break;
        case 'tradeoff_explanation':
            if (!output.explanation) {
                ...
                validateTradeoffExplanationOutput(output, errors);
            }
            break;
        case 'revision':
            if (!output.revision) {
                ...
                validateRevisionOutput(output, errors);
            }
            break;
        default:
            errors.push({
 ...
 */
function generateRetryPrompt(errors) {
...
//# sourceMappingURL=output-enforcer.js.map