...
exports.generateAssurance = generateAssurance;
exports.buildAssuranceRecord = buildAssuranceRecord;
const crypto_1 = require("crypto");
const types_1 = require("./types");
const decision_store_1 = require("../db/decision-store");
 ...
 */
function generateAssuranceId() {
    return `asr_${(0, crypto_1.randomUUID)().replace(/-/g, '').substring(0, 12)}`;
}
/**
 ...
 */
function buildInvalidationChecklist(decision) {
    const checklist = [];
 ...
 */
function validateDecisionForAssurance(decision) {
    ...
    if (decision.confidence < types_1.ASSURANCE_CONFIDENCE_THRESHOLD) {
        return {
            valid: false,
 ...
 */
async function generateAssurance(request) {
    // Fetch the decision
    const decision = await (0, decision_store_1.getDecision)(request.decision_id);
    if (!decision) {
        return {
            success: false,
 ...
 */
function buildAssuranceRecord(artifact, sessionId, travelerId) {
    ...
    return {
        ...
        payment_status: 'pending',
        payment_id: null,
        payment_amount_cents: types_1.ASSURANCE_PRICING.BASE_PRICE_CENTS,
        payment_currency: types_1.ASSURANCE_PRICING.CURRENCY,
        download_count: 0,
